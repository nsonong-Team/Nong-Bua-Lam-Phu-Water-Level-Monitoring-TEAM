...
function initSheets() {
  ...
  if (!current) current = ss.insertSheet('CurrentLevels');
  ...
  if (!config) config = ss.insertSheet('StationConfig');
  ...
  Logger.log('✅ เรียบร้อย! CurrentLevels และ StationConfig มีข้อมูลแล้ว');
}
